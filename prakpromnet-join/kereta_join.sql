...
CREATE TABLE `gerbong` (
  `kd_gerbong` varchar(15) NOT NULL,
  `kd_kereta` varchar(15) NOT NULL,
...
ALTER TABLE `gerbong`
  ...
  ADD KEY `fk_lokomotif` (`kd_kereta`);
...
--
ALTER TABLE `gerbong`
  ADD CONSTRAINT `fk_lokomotif` FOREIGN KEY (`kd_kereta`) REFERENCES `lokomotif` (`kd_kereta`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `kursi`
--
ALTER TABLE `kursi`
  ADD CONSTRAINT `fk_gebong` FOREIGN KEY (`kd_gerbong`) REFERENCES `gerbong` (`kd_gerbong`) ON DELETE CASCADE ON UPDATE CASCADE,
